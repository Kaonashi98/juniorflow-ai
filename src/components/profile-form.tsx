...
import type { DeveloperProfile, DeveloperRole, WorkTicket } from "@/types";
...
export function ProfileForm() {
  ...
  const [simulation, setSimulation] = useState<{ profile: DeveloperProfile; ticket: WorkTicket } | null>(null);
  ...
  if (simulation) {
    return (
      <section className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-8 sm:py-12">
        <div className="mb-6 flex flex-col gap-4 border border-[#cfe0aa] bg-[#f4f9e8] p-4 sm:flex-row sm:items-center sm:justify-between">
          <div><p className="text-sm font-semibold text-[#526d14]">Real ticket · Generated by GPT-5.6</p><p className="mt-1 text-sm text-[#64736d]">Your ticket is ready. Submit your solution when you are done.</p></div>
          <button type="button" onClick={() => setSimulation(null)} className="inline-flex items-center gap-2 text-sm font-semibold text-[#52615b] hover:text-[#14261f]"><ArrowLeft aria-hidden="true" size={16} />Create another ticket</button>
        </div>
        <div className="grid gap-7 xl:grid-cols-[minmax(0,1.16fr)_minmax(380px,0.84fr)] xl:items-start">
          <TicketDetails ticket={simulation.ticket} />
          <SolutionWorkspace ticket={simulation.ticket} />
        </div>
      </section>
    );
  }
