...
export function ProfileForm() {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setIsLoading(true);
    window.setTimeout(() => router.push("/demo"), 900);
  }
  ...
  return (
    <section className="mx-auto max-w-5xl px-5 py-10 sm:px-8 sm:py-14">
      <form onSubmit={handleSubmit} className="grid gap-6 lg:grid-cols-[1fr_280px] lg:items-start">
          ...
          <div className="space-y-7 p-6 sm:p-8">
            <div className="grid gap-6 sm:grid-cols-2">
              <label className="block text-sm font-semibold">
                ...
                <select name="experience" className={fieldClass} defaultValue="6–12 months">
              ...
              <label className="block text-sm font-semibold">
                ...
                <select name="availableTime" className={fieldClass} defaultValue="2 hours">
            ...
            <label className="block text-sm font-semibold">
              ...
              <select name="ticketLanguage" className={fieldClass} defaultValue="English">
            ...
            <label className="block text-sm font-semibold">
              ...
              <textarea name="projectDescription" required maxLength={800} rows={5} className={`${fieldClass} py-3`} defaultValue="A project-management dashboard for small remote teams. Users can create projects, invite teammates, and track tasks." />
              <span className="mt-1 block text-right text-xs font-normal text-[#8a9690]">Max 800 characters</span>
            </label>
          </div>
        </div>
        ...
        <aside className="border border-[#d5ddd6] bg-[#eef1e9] p-5 lg:sticky lg:top-24">
          ...
          <dl className="mt-5 space-y-4 text-sm">
            ...
            <div><dt className="text-[#718079]">Output</dt><dd className="mt-1 font-semibold">1 realistic work ticket</dd></div>
          </dl>
          <div className="mt-6 border-t border-[#d5ddd6] pt-5">
            <p className="mb-4 text-xs leading-5 text-[#718079]">This preview uses demo data. AI generation will be connected in the next phase.</p>
            <button type="submit" disabled={isLoading || technologies.length === 0} className="inline-flex min-h-12 w-full items-center justify-center gap-2 bg-[#14261f] px-4 font-semibold text-white transition-colors hover:bg-[#29483b] disabled:cursor-not-allowed disabled:opacity-60">
              {isLoading ? <><LoaderCircle aria-hidden="true" size={18} className="animate-spin" />Preparing demo…</> : <>Generate my ticket <ArrowRight aria-hidden="true" size={18} /></>}
            </button>
          </div>
        </aside>
      </form>
