...
export function LocalizedPageIntro({ eyebrow, title, description, width = "max-w-5xl", compact = false }: { eyebrow: MessageKey; title: MessageKey; description: MessageKey; width?: string; compact?: boolean }) {
  const { t } = useLanguage();
  return (
    <section className="border-b border-[#dce2dc] bg-white">
      <div className={`mx-auto ${width} px-5 ${compact ? "py-7 sm:py-9" : "py-12 sm:py-16"}`}>
